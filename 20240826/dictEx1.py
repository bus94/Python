...
# 딕셔너리 자체도 다 ~ 저장가능
dict2['program'] = {'python' : '상', 'c#' : '하', 'c++' : '중'}
print(dict2['program']['python'])
...
dict3[10] = "이서희"
print(dict3[10])
# 딕셔너리는 따로 추가하거나 수정하는 것이 똑같다.
# key를 작성해서 실행을 하면 만약 key 딕셔너리 변수에 있다면 값을 수정
# key 없으면 새로운 객체를 생성한다.
dict3[10] = (1, 2, 34)
print(dict3[10])

del dict3

# key는 중복적이면 안된다. (고유한 key)
# 변경되는 변수는 넣지 못하고, list도 넣지 못한다.
# 고정된 문자, 숫자, 튜플 - 나머지는 안된다.
...
for x in seohee.values():
    print(x) # 아래의 코드와 같은 것을 출력
print("2번 반복문")
for x in seohee:
    print(seohee[x])
print("3번 반복문")
# key와 value 쌍으로 가져올 경우
